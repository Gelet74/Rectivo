...
namespace recTivo.MVVM
{
    public class MVCliente : MVBase
    {
        ...
        // ← CAMBIO: backing field en lugar de propiedad calculada
        private List<string> _nombresClientes = new();
        public List<string> NombresClientes
        {
            get => _nombresClientes;
            set => SetProperty(ref _nombresClientes, value);
        }

        private string? _nombreClienteSeleccionado;
        public string? NombreClienteSeleccionado
        {
            get => _nombreClienteSeleccionado;
            set
            {
                SetProperty(ref _nombreClienteSeleccionado, value);
                ClienteSeleccionado = ListaClientes.FirstOrDefault(c => c.NombreCompleto == value);
            }
        }
        ...
        {
            get => _nombre;
            set { SetProperty(ref _nombre, value); OnPropertyChanged(nameof(HasNoErrors)); }
        }
        ...
        {
            get => _apellido1;
            set { SetProperty(ref _apellido1, value); OnPropertyChanged(nameof(HasNoErrors)); }
        }
        ...
        {
            get => _dni;
            set { SetProperty(ref _dni, value); OnPropertyChanged(nameof(HasNoErrors)); }
        }
        ...
        {
            get => _filtroNombre;
            set { SetProperty(ref _filtroNombre, value); ClientesView?.Refresh(); }
        }
        ...
        public string? FiltroApellido1
        {
            ...
            set { SetProperty(ref _filtroApellido1, value); ClientesView?.Refresh(); }
        }
        ...
        public string? FiltroApellido2
        {
            ...
            set { SetProperty(ref _filtroApellido2, value); ClientesView?.Refresh(); }
        }
        ...
        {
            get => _DNI;
            set { SetProperty(ref _DNI, value); ClientesView?.Refresh(); }
        }
        ...
        {
            get => _usuario;
            set { SetProperty(ref _usuario, value); OnPropertyChanged(nameof(HasNoErrors)); }
        }
        ...
        {
            get => _password;
            set { SetProperty(ref _password, value); OnPropertyChanged(nameof(HasNoErrors)); }
        }

        public List<string> NombreLista => ListaClientes
            .Select(a => a.Nombre).Where(c => !string.IsNullOrWhiteSpace(c))
            .Select(c => c!).Distinct().OrderBy(c => c).ToList();

        public List<string> Apellido1Lista => ListaClientes
            .Select(a => a.Apellido1).Where(d => !string.IsNullOrWhiteSpace(d))
            .Select(d => d!).Distinct().OrderBy(d => d).ToList();

        public List<string> Apellido2Lista => ListaClientes
            .Select(a => a.Apellido2).Where(d => !string.IsNullOrWhiteSpace(d))
            .Select(d => d!).Distinct().OrderBy(d => d).ToList();
        ...
        public async Task Inicializa()
        {
            try
            {
                ...
                // ← CAMBIO: asignación directa en lugar de OnPropertyChanged
                NombresClientes = ListaClientes
        ...
        private bool FiltarClientes(object obj)
        {
            ...
            bool coincideApellido1 = string.IsNullOrWhiteSpace(FiltroApellido1) ||
                (cliente.Apellido1?.Contains(FiltroApellido1, StringComparison.OrdinalIgnoreCase) ?? false);
            bool coincideApellido2 = string.IsNullOrWhiteSpace(FiltroApellido2) ||
        ...
        {
            if (ClienteSeleccionado == null) return Task.FromResult(false);
